...
- **pyright is not a passing gate.** The code inherited from upstream v1.2.0 has ~1900 strict-mode errors against the pinned `types-lxml` stubs. CI reports the count without failing. Annotate new code properly, but do not expect a clean run and do not "fix" inherited errors as a side quest.
