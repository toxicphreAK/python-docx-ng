...
Each section object has a `.header` property providing access to a [_Header][docx.section._Header] object for that section:
...
```
A [_Header][docx.section._Header] object is *always* present on `Section.header`, even when no header is defined for that section. The presence of an actual header definition is indicated by `_Header.is_linked_to_previous`:

```pycon
>>> header.is_linked_to_previous
True
```
A value of `True` indicates the [_Header][docx.section._Header] object contains no header definition and the section will display the same header as the previous section. This "inheritance" behavior is recursive, such that a "linked" header actually gets its definition from the first prior section having a header definition. This "linked" state is indicated as *"Same as previous"* in the Word UI.
...
A header can be added to a new document simply by editing the content of the [_Header][docx.section._Header] object. A [_Header][docx.section._Header] object is a "story" container and its content is edited just like a [Document][docx.api.Document] object. Note that like a new document, a new header already contains a single (empty) paragraph:

```pycon
>>> paragraph = header.paragraphs[0]
>>> paragraph.text = "Title of my document"
```
![image](../assets/img/hdrftr-01.png)
...
```
![image](../assets/img/hdrftr-02.png)
