...
[Run][docx.text.run.Run] objects have both a `.bold` and `.italic` property that allows you to set their value for a run:
...
Like paragraph styles, a character style must already be defined in the document you open with the `Document()` call (*see* [understanding styles](styles-understanding.md)).
